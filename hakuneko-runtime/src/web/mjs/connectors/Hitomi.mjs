...
export default class Hitomi extends Connector {
    ...
    async _getMangaFromURI(uri) {
        ...
        `;
        const rawTitle = await Engine.Request.fetchUI(request, script);
        const safeTitle = typeof rawTitle === "string" ? rawTitle.trim() : String(rawTitle || "").trim();
        const id = uri.pathname.match(/(\d+)\.html$/)[1];
        return new Manga(this, id, safeTitle || `Hitomi #${id}`);
    }
