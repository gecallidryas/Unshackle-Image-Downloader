...
(function () {
  ...
  const DEDUPE_AUTO_REMOVE_KEY = "__unshackle_dedupe_auto_remove";
  ...
  let AUTO_DEDUPE_SCAN_ID = null;
  const DEDUPE_STATS_CACHE = new Map();
  ...
  function stopActiveScan(reason = "") {
    ...
    if (ACTIVE_SCAN_SESSION?.dedupeScanId) {
      stopDedupeScan(ACTIVE_SCAN_SESSION.dedupeScanId);
  ...
  async function finishDedupeScan(scanId) {
  ...
  async function runScan(types) {
    ...
    if (session?.dedupeScanId) {
      await finishDedupeScan(session.dedupeScanId);
      session.dedupeScanId = null;
    }
    endScanSession(session?.id);
  }
  ...
  function resetDedupeRuntime() {
    DEDUPE_DUPLICATE_URLS.clear();
    DEDUPE_STATS_CACHE.clear();
    DEDUPE_NOTICE_SENT.clear();
    DEDUPE_ERROR_SEEN.clear();
    updateDedupeManualButton();
  }

  function getDuplicateUrlsInCache() {
  ...
  function removeDuplicatesFromCache({ log = false } = {}) {
    const urls = getDuplicateUrlsInCache();
    if (!urls.length) {
      updateDedupeManualButton();
      return 0;
    }
    const removed = removeItemsByUrl(urls);
    if (removed && log) {
      const msg = `Removed ${removed} duplicate(s).`;
      setHintMessage("hint_dedupe_removed", msg);
  ...
  function handleDedupeMessage(message) {
    ...
    if (message.type === "DEDUPE_HASH_RESULT" || message.type === "DEDUPE_PIXEL_HASH_RESULT") {
      if (message.status === "DUP" && message.url) {
        registerDedupeDuplicate(message.url);
      }
      return;
    }

    if (message.type === "DEDUPE_PERCEPTUAL_RESULT") {
      if ((message.status === "CONFIRMED" || message.status === "DUP") && message.url) {
        registerDedupeDuplicate(message.url);
      }
      return;
    }

    if (message.type === "DEDUPE_SCAN_STATS") {
      const scanId = message.scanId;
      const stats = message.stats;
      if (!scanId || !stats) return;
      DEDUPE_STATS_CACHE.set(scanId, stats);
      const dupCount = (stats.l1Dup || 0) + (stats.l2Dup || 0) + (stats.l3Dup || 0);
      if (dupCount > 0 && !DEDUPE_NOTICE_SENT.has(scanId)) {
        recordUserNotice("info", `Dedupe found ${dupCount} duplicates (L1 ${stats.l1Dup || 0}, L2 ${stats.l2Dup || 0}, L3 ${stats.l3Dup || 0}).`);
        DEDUPE_NOTICE_SENT.add(scanId);
      }
      return;
    }

    if (message.type === "DEDUPE_HASH_ERROR") {
      const scanId = message.scanId || "unknown";
      const errorKey = `${scanId}:${message.errorCode || "error"}`;
      if (DEDUPE_ERROR_SEEN.has(errorKey)) return;
      DEDUPE_ERROR_SEEN.add(errorKey);
      const detail = message.errorCode ? ` (${message.errorCode})` : "";
      recordUserNotice("warn", `Dedupe error${detail}.`);
    }
  }
    ...
    if (on) {
      ...
      try {
        ...
        if (AUTO_DEDUPE_SCAN_ID) {
          finishDedupeScan(AUTO_DEDUPE_SCAN_ID);
  ...
  async function loadDedupePreference() {
    try {
      const prefs = await chrome.storage.sync.get({ [DEDUPE_AUTO_REMOVE_KEY]: true });
      autoRemoveDuplicates = prefs[DEDUPE_AUTO_REMOVE_KEY] !== false;
    } catch {
      autoRemoveDuplicates = true;
    }
    const toggle = $("#dedupeAutoRemoveSwitch");
    if (toggle) toggle.checked = autoRemoveDuplicates;
    updateDedupeManualButton();
  }
  ...
  function bindDedupePreference() {
    const toggle = $("#dedupeAutoRemoveSwitch");
    if (!toggle) return;
    toggle.addEventListener("change", async (event) => {
      await setDedupePreference(!!event.target.checked);
    });
  }
  ...
  function bindDownloadButtons() {
    ...
    const dedupeBtn = $("#dedupeManual");
    if (dedupeBtn) {
      dedupeBtn.addEventListener("click", () => {
        const removed = removeDuplicatesFromCache({ log: true });
        if (!removed) {
          const msg = "No duplicates to remove.";
          setHintMessage("hint_dedupe_empty", msg);
          recordUserNotice("info", msg);
        }
      });
    }
  ...
  if (chrome.runtime?.onMessage) {
    chrome.runtime.onMessage.addListener((message, sender) => {
      if (!message) return;
      if (typeof message.type === "string" && message.type.startsWith("DEDUPE_")) {
        handleDedupeMessage(message);
        return;
      }
