(() => {
  ...
  const DEDUPE_MESSAGE_TYPES = {
    CANDIDATES: "DEDUPE_CANDIDATES",
    BLOB_BYTES: "DEDUPE_BLOB_BYTES"
  };
  ...
  async function getBlobBytesForDedupe(blobUrl) {
    if (typeof blobUrl !== "string" || !blobUrl.startsWith("blob:")) return null;
    const reg = BLOB_REG.get(blobUrl);
    if (reg && reg.buffer instanceof ArrayBuffer) {
      try {
        return reg.buffer.slice(0);
      } catch {
        return reg.buffer;
      }
    }
    const fetched = await blobUrlToArrayBuffer(blobUrl, DEDUPE_BLOB_SIZE_LIMIT);
    if (!fetched || fetched.oversized || !fetched.buffer) return null;
    return fetched.buffer;
  }

  async function sendDedupeCandidates(items, options) {
    const dedupeScanId = typeof options?.dedupeScanId === "string" ? options.dedupeScanId : null;
    if (!dedupeScanId || !Array.isArray(items) || !items.length) return;
    const tabId = Number.isInteger(options?.dedupeTabId) ? options.dedupeTabId : null;
    const pageUrl = typeof options?.dedupePageUrl === "string" ? options.dedupePageUrl : (location?.href || "");
    const candidates = [];
    const blobJobs = [];

    for (const item of items) {
      const url = getDedupeUrl(item);
      if (!url) continue;
      const context = buildDedupeContext(item);
      if (url.startsWith("blob:")) {
        blobJobs.push({ url, context });
      } else {
    ...
    if (candidates.length) {
      void sendExtensionMessage({
        type: DEDUPE_MESSAGE_TYPES.CANDIDATES,
        scanId: dedupeScanId,
        tabId,
        pageUrl,
        candidates
    ...
    if (blobJobs.length) {
      for (const job of blobJobs) {
        try {
          const bytes = await getBlobBytesForDedupe(job.url);
          if (!bytes) continue;
          void sendExtensionMessage({
            type: DEDUPE_MESSAGE_TYPES.BLOB_BYTES,
            scanId: dedupeScanId,
            url: job.url,
            bytes,
            context: job.context
  ...
  async function scanForImagesConcurrent(options = {}) {
    ...
    const dedupeScanId = typeof options.dedupeScanId === "string" ? options.dedupeScanId : null;
    const dedupeTabId = Number.isInteger(options.dedupeTabId) ? options.dedupeTabId : null;
    const dedupePageUrl = typeof options.dedupePageUrl === "string" ? options.dedupePageUrl : (location?.href || "");
    const dedupeOptions = dedupeScanId ? { dedupeScanId, dedupeTabId, dedupePageUrl } : null;
    ...
    const flushIncremental = (force = false, label = null) => {
      ...
      if (dedupeOptions) {
        void sendDedupeCandidates(pendingChunk.slice(), dedupeOptions);
      }
      lastEmit = now;
      pendingChunk = [];
