...
export default class MangaDex extends Connector {
    ...
    async _getChapters(manga) {
        console.log(`[MangaDex] Fetching chapters for manga: ${manga.title || manga.id}`);
        let chapterList = [];
        for (let page = 0, run = true; run; page++) {
            try {
                let chapters = await this._getChaptersFromPage(manga, page);
                console.log(`[MangaDex] Page ${page}: fetched ${chapters.length} chapter(s)`);
                chapters.length > 0 ? chapterList.push(...chapters) : run = false;
            } catch (error) {
                console.error(`[MangaDex] Failed to fetch page ${page}:`, error);
                run = false;
            }
        }
        console.log(`[MangaDex] Total chapters fetched: ${chapterList.length}`);
    ...
    async _getChaptersFromPage(manga, page) {
        ...
        try {
            ...
            console.error(`[MangaDex] Error fetching chapters from page ${page}:`, error);
            throw error;
        }
    }
