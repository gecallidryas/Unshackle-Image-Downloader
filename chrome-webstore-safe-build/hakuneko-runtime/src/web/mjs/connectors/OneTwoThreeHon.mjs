...
export default class OneTwoThreeHon extends SpeedBinb {
    ...
    constructor() {
        ...
        this.mangaList = [
            { domain: 'https://www.123hon.com/polca', path: '/web-comic/' },
            { domain: 'https://www.123hon.com/nova', path: '/web-comic' }
        ];
    ...
    async _getMangas() {
        const mangalist = [];
        for (const { domain, path } of this.mangaList) {
            const base = new URL(domain);
            const request = new Request(new URL(path, base), this.requestOptions);
            const data = await this.fetchDOM(request, this.queryMangas);
            const mangas = data.map(link => {
                return {
                    id: new URL(link.getAttribute('href'), base).href,
