...
export default class CoreView extends Connector {
    ...
    async _handleConnectorURI(payload) {
        /**
         * NOTE: Do NOT descramble here!
         * 
         * The panel.js (descrambleHKCoreViewImage) and background.js (descrambleCoreViewImage)
         * already handle descrambling for the CoreView/GigaViewer family based on the `family`
         * metadata returned with pages.
         * 
         * If we descramble here AND they descramble again, we get double-descramble which
         * re-scrambles the image (the algorithm is symmetric - applying it twice returns
         * to the original scrambled state).
         * 
         * By skipping descrambling here, the flow becomes:
         * - Native connector returns scrambled image via connector:// URL
         * - Panel/background detects CoreView family and descrambles once
         * - Result: correctly descrambled image
         */
        let request = new Request(payload, this.requestOptions);
        let response = await fetch(request);
        let data = await response.blob();
        // Descrambling is handled by panel.js/background.js based on family detection
