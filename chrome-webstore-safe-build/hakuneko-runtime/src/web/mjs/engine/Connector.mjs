...
export default class Connector {
     ...
     */
    createDOM( content, replaceImageTags, clearIframettributes, stripExternalScripts ) {
        ...
        stripExternalScripts = stripExternalScripts !== undefined ? stripExternalScripts : true;
        if( replaceImageTags ) {
        ...
        if( stripExternalScripts ) {
            content = content.replace( /<script\b[^>]*\bsrc\s*=\s*["'][^"']+["'][^>]*>[\s\S]*?<\/script>/gi, '' );
        }
    ...
    async fetchPROTO(request, protoTypes, rootType) {
        let resolvedPath = protoTypes;
        if (typeof protoTypes === 'string' && protoTypes.startsWith('/mjs/')) {
            const relativePath = `hakuneko-runtime/src/web${protoTypes}`;
            const baseURI = typeof chrome !== 'undefined' && chrome.runtime?.getURL
                ? chrome.runtime.getURL(relativePath)
                : relativePath;
            resolvedPath = baseURI;
        }
        let Root = (await protobuf.load(resolvedPath)).lookupType(rootType);
        let response = await fetch(request);
